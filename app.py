...
with tabs[1]:
    if 'df' not in st.session_state:
        ...
        st.markdown("""<div style="background:#1a1f35;border-radius:12px;padding:20px;border:1px solid rgba(255,255,255,0.08);margin-bottom:20px">
            ...
            🟢 وضع البيانات السليمة في الأعلى<br>
            🔴 وضع البيانات الناقصة في الأسفل مع ملاحظة توضح وش الناقص<br>
        ...
        if st.button("🧹 نظّف البيانات الآن", use_container_width=True):
            ...
            if not clean_rows.empty:
                st.dataframe(clean_rows.style.applymap(lambda x: 'background-color: #1B5E20; color: white'), use_container_width=True)
            if not dirty_rows.empty:
                st.markdown("**🔴 البيانات الناقصة**")
                st.dataframe(dirty_rows.style.applymap(lambda x: 'background-color: #B71C1C; color: white'), use_container_width=True)

            st.markdown('<div class="section-title">📥 تصدير الملف المنظم</div>', unsafe_allow_html=True)
